...
//temp, change to 2400
#define TIMEOUT "2400"	// 24s

//#define DEBUG_STREAM
#ifdef DEBUG_STREAM

#define MAX_STREAM_LEN  1024

static void
dump_hex_str(struct skynet_context * ctx, unsigned char * buf, int sz) {
    char buffer[MAX_STREAM_LEN*3+1];
    int n = sz > MAX_STREAM_LEN ? MAX_STREAM_LEN : sz;
    int i, itr = 0;
    for (i=0; i<n; ++i) {
        itr += sprintf(buffer+itr, "%02x ", buf[i]);
    }
    skynet_error(ctx, buffer);
}
...
socket_message(struct skynet_context *ctx, struct package *P, const struct skynet_socket_message * smsg) {
	...
	case SKYNET_SOCKET_TYPE_DATA:
        skynet_error(ctx, "Recv %d bytes from %d", smsg->ud, P->fd);
...
#ifdef DEBUG_STREAM
    dump_hex_str(ctx, p, sz+2);
#endif
    skynet_error(ctx, "Send %d bytes to %d", sz, P->fd);
}
